...
test.describe('N8N Webhook Communication Tests', () => {
    ...
    test.describe('JSON Payload Validation Tests', () => {
        test('should validate complete payload structure', async ({ page }) => {
            ...
            expect(capturedPayload.timestamp).toMatch(
        ...
        test('should handle special characters and unicode in messages', async ({ page }) => {
            ...
            const specialCharsMessage =
                'Search for "Société Générale" executives with résumés containing €100M+ deals';
    ...
    test.describe('SSE Streaming Response Tests', () => {
        test('should handle Server-Sent Events streaming responses', async ({ page }) => {
            ...
            const streamingMessages = await page.evaluate(
                () => (window as any).streamingMessages || []
            );
                ...
                if (connectionCount === 1) {
                    ...
                    const stream = new ReadableStream({
                        start(controller) {
                            controller.enqueue(
                                new TextEncoder().encode(
                                    `data: ${JSON.stringify({ message: 'Starting search...' })}\n\n`
                                )
                            );
                            ...
                            setTimeout(() => {
                                controller.enqueue(
                                    new TextEncoder().encode(
                                        `data: ${JSON.stringify({ message: 'Found initial results...' })}\n\n`
                                    )
                                );
                            }, 1000);
                    ...
                    const stream = new ReadableStream({
                        start(controller) {
                            controller.enqueue(
                                new TextEncoder().encode(
                                    `data: ${JSON.stringify({ message: 'Reconnected. Continuing search...' })}\n\n`
                                )
                            );
                            ...
                            setTimeout(() => {
                                controller.enqueue(
                                    new TextEncoder().encode(
                                        `data: ${JSON.stringify({ message: 'Search completed successfully.' })}\n\n`
                                    )
                                );
                ...
                const stream = new ReadableStream({
                    start(controller) {
                        ...
                        const events = [
                            ...
                            {
                                event: 'progress',
                                data: { message: 'Processing step 1 of 3', progress: 33 },
                            },
                            {
                                event: 'progress',
                                data: { message: 'Processing step 2 of 3', progress: 66 },
                            },
                            {
                                event: 'result',
                                data: {
    ...
    test.describe('Error Handling Tests', () => {
        ...
        test('should implement retry mechanism for temporary failures', async ({ page }) => {
            ...
            await expect(page.locator(':text("Request succeeded after retry")')).toBeVisible({
                timeout: 15000,
            });
        });
    ...
    test.describe('Performance and Timeout Tests', () => {
        ...
        test('should measure and log response times', async ({ page }) => {
            ...
            const clientResponseTimes = await page.evaluate(
                () => (window as any).responseTimes || []
            );
            ...
            const averageTime =
                clientResponseTimes.reduce((a: number, b: number) => a + b, 0) /
                clientResponseTimes.length;

...
test.describe('Prompt-Based Test Scenarios', () => {
  ...
  test.describe('Jet Charter Operations Scenarios (jet-1 to jet-4)', () => {
    test('jet-1: Aircraft Availability - Miami to New York tomorrow', async ({ page }) => {
      const scenario = {
        ...
        expectedResponse: /aircraft|availability|Miami|New York|Gulfstream|Citation|hourly rate/i
      };
    ...
    test('jet-2: Empty Legs - Find opportunities for this weekend', async ({ page }) => {
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /northeast|corridor|50%|discount|passenger/i);
      }
    });
    ...
    test('jet-3: Fleet Utilization - Analyze metrics for this month', async ({ page }) => {
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /underutilized|benchmark|optimization|strategy/i);
      }
    });
    ...
    test('jet-4: Heavy Jets - Search for 12 passengers to London Tuesday', async ({ page }) => {
      const scenario = {
        ...
        expectedResponse: /heavy jet|12 passenger|London|Tuesday|Gulfstream|G650|transatlantic|6000nm/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /catering|ground transport|total cost|international fee|safety rating/i);
      }
    });
  });

  test.describe('Apollo Campaign Management Scenarios (apollo-1 to apollo-4)', () => {
    test('apollo-1: Weekly Conversions - Analyze prospect to booking conversions', async ({ page }) => {
      const scenario = {
        id: 'apollo-1',
        category: 'Apollo',
        ...
        expectedResponse: /conversion|prospect|booking|funnel|open rate|meeting|ROI|analytics/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /industry|vertical|campaign|performance|previous week/i);
      }
    });

    test('apollo-2: EA Engagement - Identify top engaged executive assistants', async ({ page }) => {
      const scenario = {
        id: 'apollo-2',
        category: 'Apollo',
        ...
        expectedResponse: /executive assistant|engaged|email engagement|top 50|opens|clicks|replies/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /private equity|content|engagement|personalized|outreach/i);
      }
    });

    test('apollo-3: Finance Campaigns - Analyze finance sector performance', async ({ page }) => {
      const scenario = {
        id: 'apollo-3',
        category: 'Apollo',
        ...
        expectedResponse: /finance|sector|campaign|performance|investment banking|private equity|hedge fund/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /investment banking|private equity|subject line|seasonal|earnings/i);
      }
    });

    test('apollo-4: VIP Campaign - Design for top 100 qualified prospects', async ({ page }) => {
      const scenario = {
        id: 'apollo-4',
        category: 'Apollo',
        ...
        expectedResponse: /VIP|campaign|top 100|qualified|prospects|multi-channel|sequence|personalized/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /behavioral|trigger|tracking|VIP|offer|Slack|notification/i);
      }
    });
  });

  test.describe('Travel Planning & Coordination Scenarios (travel-1 to travel-4)', () => {
    test('travel-1: Multi-City Planning - Tech executive roadshow across 5 cities', async ({ page }) => {
      const scenario = {
        id: 'travel-1',
        category: 'Travel',
        ...
        expectedResponse: /tech executive|roadshow|5 cities|SF|Seattle|Austin|NYC|Boston|7 days/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /time zone|jet lag|FBO|conference|master itinerary|mobile/i);
      }
    });
    ...
    test('travel-2: Weather Routes - Optimize to avoid delays this week', async ({ page }) => {
      const scenario = {
        ...
        expectedResponse: /optimize|routes|weather|delay|week|forecast|turbulence|alternate/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /east coast|alternate|routing|turbulence|forecast|ride quality/i);
      }
    });

    test('travel-3: Industry Patterns - Analyze entertainment travel trends', async ({ page }) => {
      const scenario = {
        id: 'travel-3',
        category: 'Travel',
        ...
        expectedResponse: /entertainment|industry|seasonal|travel|trends|award season|festival|production/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /award season|travel pattern|festival|circuit|production|schedule/i);
      }
    });
    ...
    test('travel-4: Ground Transport - Arrange for tomorrow\'s flights', async ({ page }) => {
      const scenario = {
        ...
        expectedResponse: /ground transportation|tomorrow|luxury vehicle|chauffeur|meet greet|flight tracking/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /executive security|pet transportation|real-time|flight tracking/i);
      }
    });
  });
  ...
  test.describe('Lead Generation & Targeting Scenarios (lead-1 to lead-4)', () => {
    test('lead-1: PE Assistants - Find at NYC private equity firms', async ({ page }) => {
      const scenario = {
        ...
        expectedResponse: /executive assistant|NYC|private equity|firm|Partner|MD|C-suite|AUM/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /1B|AUM|executive|travel pattern|Apollo|campaign|personalized/i);
      }
    });
    ...
    test('lead-2: Job Changes - Track in target accounts last 30 days', async ({ page }) => {
      const scenario = {
        ...
        expectedResponse: /job change|target account|30 days|new hire|promotion|VP|executive assistant|travel manager/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /high-growth|fresh funding|segmented|engagement|congratulations|sequence/i);
      }
    });
    ...
    test('lead-3: Decision Makers - Map at Fortune 500 companies', async ({ page }) => {
      const scenario = {
        ...
        expectedResponse: /decision maker|Fortune 500|economic buyer|user buyer|technical buyer|influencer|champion/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /economic buyer|budget authority|reporting structure|hierarchy|multi-threaded/i);
      }
    });
    ...
    test('lead-4: Web Visitors - Identify high-intent from pricing page', async ({ page }) => {
      const scenario = {
        ...
        expectedResponse: /high-intent|visitor|pricing page|lead scoring|calculator|company match|intent score/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /calculator|usage pattern|Apollo|profile|automated|nurture|high-intent/i);
      }
    });
  });
  ...
  test.describe('Analytics & Insights Scenarios (analytics-1 to analytics-4)', () => {
    test('analytics-1: Conversion Trends - Compare with last quarter', async ({ page }) => {
      const scenario = {
        ...
        expectedResponse: /conversion rate|last quarter|funnel|lead to MQL|MQL to SQL|opportunity|closed won/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /lead source|industry|statistically significant|project|end-of-quarter/i);
      }
    });

    test('analytics-2: Campaign ROI - Calculate by type and industry vertical', async ({ page }) => {
      const scenario = {
        id: 'analytics-2',
        category: 'Analytics',
        ...
        expectedResponse: /ROI|campaign type|industry vertical|costs|revenue|LTV|CAC|payback|attribution/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /attribution|model|lifetime value|segment|optimization|recommendation|projected/i);
      }
    });

    test('analytics-3: Message Performance - Analyze email templates and messaging', async ({ page }) => {
      const scenario = {
        id: 'analytics-3',
        category: 'Analytics',
        ...
        expectedResponse: /email template|messaging|open rate|click-through|reply rate|A\/B test|optimization/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /A\/B test|subject line|power word|engagement|optimization|roadmap|expected lift/i);
      }
    });

    test('analytics-4: Executive Briefing - Generate Monday morning report', async ({ page }) => {
      const scenario = {
        id: 'analytics-4',
        category: 'Analytics',
        ...
        expectedResponse: /executive briefing|Monday morning|weekend highlight|week ahead|KPI|strategic update|action item/i
      };
      ...
      for (const followUp of followUps) {
        await testFollowUpQuestion(page, followUp, /customer communication|fleet utilization|revenue per hour|blocker|executive attention/i);
      }
    });
  });
  ...
  test.describe('Session Memory and Context Retention', () => {
    test('should maintain context across multi-turn conversations', async ({ page }) => {
      ...
      await page.route(WEBHOOK_URL + '**', async (route, request) => {
        ...
        } else if (sessionContext.length === 2 && payload.message.toLowerCase().includes('pricing')) {
          response = 'Pricing for the G650 options: N123GS at $8,500/hr, N456GS at $9,200/hr, N789GS at $8,800/hr';
        } else if (sessionContext.length === 3 && payload.message.toLowerCase().includes('book')) {
          response = 'Which specific G650 would you prefer? Please specify the tail number or hourly rate.';
        } else if (sessionContext.length === 4) {
          response = 'Booking confirmed for N123GS (Gulfstream G650) at $8,500/hr for Miami to NYC tomorrow';
        }
    ...
    test('should handle complex parameter extraction from prompts', async ({ page }) => {
      ...
      await page.route(WEBHOOK_URL + '**', async (route, request) => {
        ...
        if (message.includes('gulfstream g650') && message.includes('12 passengers') && message.includes('london')) {
          params = {
      ...
      const complexPrompt = 'Search heavy jets like Gulfstream G650 for 12 passengers to London next Tuesday with transatlantic range, WiFi, and catering';
  ...
  async function testPromptScenario(page: Page, scenario: any) {
    ...
    if (scenario.expectedResponse) {
      const responseText = await page.locator('[data-testid="chat-message"]').nth(1).textContent();

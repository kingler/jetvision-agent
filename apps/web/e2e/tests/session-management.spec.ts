...
test.describe('Session Management Tests', () => {
  ...
  test.describe('Session Creation Tests', () => {
    ...
    test('should maintain consistent session IDs within same conversation', async ({ page }) => {
      const capturedRequests: any[] = [];
    ...
    test('should persist session context across multiple requests', async ({ page }) => {
      ...
      await page.route(WEBHOOK_URL + '**', async (route, request) => {
        ...
        sessionHistory.push({
          ...
          messageCount: sessionHistory.filter(h => h.sessionId === payload.sessionId).length + 1
        });
        ...
        await route.fulfill({
          ...
          body: JSON.stringify({
            ...
            sessionHistory: sessionHistory.filter(h => h.sessionId === payload.sessionId),
            status: 'success'
          })
        });
      });
      ...
      for (let i = 0; i < conversationMessages.length; i++) {
        ...
        const sessionMessages = sessionHistory.filter(h => h.sessionId === sessionHistory[0].sessionId);
  ...
  test.describe('Postgres Chat Memory Tests', () => {
    ...
    test('should retrieve and use chat memory for context', async ({ page }) => {
      ...
      await page.route(WEBHOOK_URL + '**', async (route, request) => {
        ...
        const contextualResponse = history.length > 1 

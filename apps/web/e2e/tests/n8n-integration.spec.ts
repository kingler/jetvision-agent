...
test.describe('N8N Integration Tests', () => {
  ...
  test('should detect aircraft availability intent', async ({ page }) => {
    ...
    const testQuery = 'Check availability of Gulfstream G650 for charter from KJFK to EGLL next week';
    await page.fill('[contenteditable="true"]', testQuery);
    ...
    if (webhookRequest) {
      ...
      if (postData) {
        ...
        if (queryData) {
          ...
          expect(parsedPayload.intent.entities.some((e: string) => e.includes('Gulfstream'))).toBeTruthy();
        }
      }
    }
  });
  ...
  test('should correctly classify Apollo.io queries', async ({ page }) => {
    ...
    const intentResults = await page.evaluate(() => {
      ...
      const detectIntent = (message: string) => {
        ...
        if (lowerMessage.includes('apollo') || lowerMessage.includes('contact') || lowerMessage.includes('lead')) {
          return {

...
test.describe('Apollo.io Integration Tests', () => {
  ...
  test.describe('Lead Generation Workflows', () => {
    test('should find executive assistants at Fortune 500 companies', async ({ page }) => {
      ...
      // Check response time
      await aviationAssertions.assertResponseTime(startTime, query.timeout!, 'Apollo lead search');

      // Verify lead results structure
      const responseMessage = page.locator('[data-testid="chat-message"]').last();
      const leadElements = responseMessage.locator('[data-testid="lead-result"], .lead-item, [class*="lead"]');
    ...
    test('should handle bulk lead enrichment', async ({ page }) => {
      ...
      await chatInput.fill('Enrich these 5 contacts with full profile data and email verification');
      await chatInput.press('Enter');
      ...
      const responseMessage = page.locator('[data-testid="chat-message"]').last();
      const enrichedContacts = responseMessage.locator('[data-testid="enriched-contact"], .contact-enriched');
  ...
  test.describe('Campaign Management', () => {
    test('should show campaign performance metrics', async ({ page }) => {
      ...
      const metricsSection = responseMessage.locator('[data-testid="campaign-metrics"], .campaign-stats, [class*="metrics"]');
    ...
    test('should create and launch targeted campaigns', async ({ page }) => {
      ...
      const campaignDetails = responseMessage.locator('[data-testid="campaign-details"], .campaign-info');
      if (await campaignDetails.isVisible()) {
        await expect(campaignDetails.locator('[data-testid="campaign-name"]')).toBeVisible();
    ...
    test('should manage email sequences', async ({ page }) => {
      ...
      await chatInput.fill('Add these 50 aviation executives to our executive outreach sequence');
      await chatInput.press('Enter');
    ...
    test('should optimize campaign targeting', async ({ page }) => {
      ...
      await chatInput.fill('Analyze campaign performance and suggest optimization for aviation leads');
      await chatInput.press('Enter');
      ...
      expect(responseText?.toLowerCase()).toMatch(/(optimization|optimize|improve|suggestion)/);
      ...
      const recommendations = responseMessage.locator('[data-testid="recommendations"], .optimization-tips');
  ...
  test.describe('Contact and Account Management', () => {
    test('should create and update contact profiles', async ({ page }) => {
      ...
      await chatInput.fill('Create contact profile for John Smith, CEO at AeroTech Solutions');
      await chatInput.press('Enter');
      ...
      const contactCard = responseMessage.locator('[data-testid="contact-card"], .contact-profile');
    ...
    test('should search existing contacts with filters', async ({ page }) => {
      ...
      await chatInput.fill('Search contacts from aviation companies contacted in last 30 days');
      await chatInput.press('Enter');

      await waitForN8NResponse(page, 18000);

      const responseMessage = page.locator('[data-testid="chat-message"]').last();
      const contactResults = responseMessage.locator('[data-testid="contact-result"], .contact-item');
    ...
    test('should manage account-based marketing data', async ({ page }) => {
      ...
      const companyInfo = responseMessage.locator('[data-testid="company-info"], .account-details');
  ...
  test.describe('Data Validation and Quality', () => {
    test('should validate email addresses in lead data', async ({ page }) => {
      ...
      await chatInput.fill('Find and verify email addresses for procurement managers at aerospace companies');
      await chatInput.press('Enter');

      await waitForN8NResponse(page, 25000);

      const responseMessage = page.locator('[data-testid="chat-message"]').last();
      const emailElements = responseMessage.locator('[data-testid="contact-email"], .email-address');
    ...
    test('should handle data enrichment accuracy', async ({ page }) => {
      ...
      await chatInput.fill('Enrich contact data and validate job titles in aviation industry');
      await chatInput.press('Enter');
    ...
    test('should detect and handle duplicate contacts', async ({ page }) => {
      ...
      await chatInput.fill('Find contacts and remove duplicates from aviation executive list');
      await chatInput.press('Enter');
  ...
  test.describe('Integration Edge Cases', () => {
    test('should handle large result sets', async ({ page }) => {
      ...
      await chatInput.fill('Find all executives in aerospace industry across North America (expect 500+ results)');
      await chatInput.press('Enter');

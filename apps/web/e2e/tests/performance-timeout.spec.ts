...
test.describe('Performance and Timeout Tests', () => {
    ...
    test.describe('Response Time Measurements', () => {
        test('should measure and validate webhook response times', async ({ page }) => {
            ...
            const avgResponseTime =
                clientMetrics.reduce((sum: number, metric: any) => sum + metric.duration, 0) /
            ...
            if (avgResponseTime > IDEAL_RESPONSE_TIME) {
                console.warn(
                    `Performance warning: Average response time ${avgResponseTime.toFixed(2)}ms exceeds ideal ${IDEAL_RESPONSE_TIME}ms`
                );
            }
        });
        ...
        test('should maintain consistent performance across multiple requests', async ({
            ...
            const variance =
                performanceData.reduce((sum, val) => sum + Math.pow(val - avg, 2), 0) /
                performanceData.length;
            const stdDev = Math.sqrt(variance);
            const coefficientOfVariation = stdDev / avg;

            console.log(
                `Performance Stats - Avg: ${avg.toFixed(2)}ms, StdDev: ${stdDev.toFixed(2)}ms, CV: ${coefficientOfVariation.toFixed(3)}`
            );
    ...
    test.describe('Timeout Scenario Handling', () => {
        test('should handle webhook timeouts gracefully', async ({ page }) => {
            ...
            await expect(
                page.locator('[data-testid="timeout-error"], [data-testid="error-message"]')
            ).toBeVisible({
        ...
        test('should handle network timeouts with proper user feedback', async ({ page }) => {
            ...
            const errorMessage = await page
                .locator('[data-testid="timeout-error"], [data-testid="error-message"]')
                .textContent();
        ...
        test('should implement circuit breaker pattern for repeated failures', async ({ page }) => {
            ...
            const errorMessages = await page
                ...
                .allTextContents();
            const circuitBreakerMessage = errorMessages.find(
                msg => msg.includes('circuit breaker') || msg.includes('too many failures')
            );
        ...
        test('should handle streaming timeouts and partial responses', async ({ page }) => {
                    ...
                    start(controller) {
                        ...
                        controller.enqueue(
                            new TextEncoder().encode(
                                `data: ${JSON.stringify({ message: 'Starting process...' })}\n\n`
                            )
                        );
                        ...
                        setTimeout(() => {
                            controller.enqueue(
                                new TextEncoder().encode(
                                    `data: ${JSON.stringify({ message: 'Partial progress...' })}\n\n`
                                )
                            );
                        }, 1000);
            ...
            await expect(page.locator(':text("Starting process...")')).toBeVisible({
                timeout: 3000,
            });
            await expect(page.locator(':text("Partial progress...")')).toBeVisible({
                timeout: 3000,
            });
            ...
            const hasError = await page
                .locator('[data-testid="error-message"], [data-testid="incomplete-response"]')
                .isVisible();
    ...
    test.describe('Load Testing and Concurrent Requests', () => {
        test('should handle moderate concurrent load', async ({ page, context }) => {
            ...
            console.log(
                `Individual request times: ${requestTimes.map(t => t.toFixed(0)).join(', ')}ms`
            );
        ...
        test('should maintain UI responsiveness under load', async ({ page }) => {
            ...
            await page.route(WEBHOOK_URL + '**', async route => {
                ...
                await route.fulfill({
                    ...
                    contentType: 'application/json',
                    body: JSON.stringify({
                        message: `Load test response ${requestCount}`,
                        status: 'success',
                    }),
                });
            });
        ...
        test('should manage memory usage with large conversation histories', async ({ page }) => {
            ...
            await page.route(WEBHOOK_URL + '**', async route => {
                ...
                await route.fulfill({
                    ...
                    body: JSON.stringify({
                        message:
                            'Large response with significant content data for memory testing purposes. This message contains additional data to simulate realistic memory usage patterns in a production environment with complex responses from the N8N workflow system.',
                        status: 'success',
                    }),
                });
            });
            ...
            for (let i = 1; i <= LARGE_MESSAGE_COUNT; i++) {
                await page.fill(
                    ...
                    `Memory test message ${i} with additional content to increase memory usage`
                );
                await page.click('button[type="submit"]');
            ...
            console.log(
                `Memory usage - Baseline: ${(memoryBaseline / 1024 / 1024).toFixed(2)}MB, Peak: ${(memoryPeak / 1024 / 1024).toFixed(2)}MB`
            );
    ...
    test.describe('Streaming Performance and Latency', () => {
        test('should measure streaming latency and throughput', async ({ page }) => {
                    ...
                    constructor(url: string, options?: any) {
                        ...
                        this.addEventListener('message', event => {
                            ...
                            if ((window as any).streamingMetrics.firstByteTime === 0) {
                                (window as any).streamingMetrics.firstByteTime =
                                    now - (window as any).streamingMetrics.startTime;
                            }
            ...
            await expect(page.locator(':text("Search completed successfully")')).toBeVisible({
                timeout: 10000,
            });
        ...
        test('should handle streaming backpressure efficiently', async ({ page }) => {
            ...
            await expect(
                page.locator(`:text("Chunk ${CHUNK_COUNT} of ${CHUNK_COUNT}")`)
    ...
    test.describe('Error Recovery Performance', () => {
        test('should recover quickly from network errors', async ({ page }) => {
            ...
            // Wait for success
            await expect(page.locator(':text("Recovery successful")')).toBeVisible({
                timeout: 5000,
            });
    ...
    test.describe('Database Performance', () => {
        test('should handle large conversation history efficiently', async ({ page }) => {
            ...
            const avgDbTime =
                dbOperationTimes.reduce((sum, time) => sum + time, 0) / dbOperationTimes.length;

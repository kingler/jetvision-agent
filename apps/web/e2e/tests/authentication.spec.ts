...
test.describe('Authentication Flow Tests', () => {
  ...
  test.describe('Login Process', () => {
    test('should redirect unauthenticated users to login', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="login-container"], .login-form')).toBeVisible();
    });
    ...
    test('should login admin user successfully', async ({ page }) => {
      ...
      const adminElements = page.locator('[data-testid="admin-panel"], [data-testid="admin-controls"]');
    ...
    test('should login sales user successfully', async ({ page }) => {
      const salesUser = testUsers.find(u => u.role === 'user' && u.metadata?.department === 'Sales');
    ...
    test('should login demo user with limited access', async ({ page }) => {
      ...
      const demoIndicator = page.locator('[data-testid="demo-banner"], .demo-mode, [class*="demo"]');
    ...
    test('should handle Clerk authentication flow', async ({ page }) => {
      ...
      if (await clerkSignIn.isVisible({ timeout: 5000 })) {
        ...
        const passwordInput = clerkSignIn.locator('input[type="password"], input[name="password"]');
  ...
  test.describe('Permission Management', () => {
    ...
    test('should verify user permissions for Avinode access', async ({ page }) => {
      ...
      const avinodeFeatures = page.locator('[data-testid="avinode-search"], [class*="avinode"]');
    ...
    test('should handle session storage failures', async ({ page }) => {
      ...
      try {
        ...
        if (await errorMessage.isVisible({ timeout: 5000 })) {
          ...
          expect(errorText?.toLowerCase()).toMatch(/(storage|browser|private.*mode|cookies)/);
        }
      }
    });
  });
  ...
  test.describe('Error Scenarios', () => {
    test('should handle authentication service outage', async ({ page }) => {
      ...
      const serviceError = page.locator('[data-testid="auth-service-error"], .service-unavailable');
      await expect(serviceError).toBeVisible({ timeout: 10000 });
      
      const errorText = await serviceError.textContent();
      expect(errorText?.toLowerCase()).toMatch(/(service.*unavailable|try.*later|maintenance)/);
    });

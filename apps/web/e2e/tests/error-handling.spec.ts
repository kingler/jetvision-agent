...
test.describe('Error Handling and Fallback Tests', () => {
    ...
    test.describe('N8N Service Failures', () => {
        test('should handle N8N service unavailable gracefully', async ({ page }) => {
            ...
            const errorMessage = page.locator(
                '[data-testid="error-message"], [data-testid="fallback-message"], .error-message'
            );
            ...
            expect(errorText?.toLowerCase()).toMatch(
                /(try.*again|fallback|alternative|contact.*support)/
            );
        });
        ...
        test('should activate circuit breaker after multiple failures', async ({ page }) => {
            ...
            for (let i = 0; i < failureQueries.length; i++) {
                ...
                const circuitBreakerMessage = page.locator(
                    '[data-testid="circuit-breaker-message"], .circuit-breaker-active'
                );
                if (await circuitBreakerMessage.isVisible()) {
                    ...
                    expect(breakerText?.toLowerCase()).toMatch(
                        /(circuit.*breaker|service.*degraded|fallback.*mode)/
                    );
                    break;
                }
            }
        ...
        test('should handle N8N timeout scenarios', async ({ page }) => {
            ...
            const timeoutIndicator = page.locator(
                '[data-testid="timeout-message"], [data-testid="loading-timeout"], .timeout-error'
            );
            await expect(timeoutIndicator).toBeVisible({ timeout: 35000 });

            const timeoutText = await timeoutIndicator.textContent();
            expect(timeoutText?.toLowerCase()).toMatch(
                /(timeout|taking.*longer|please.*wait|try.*again)/
            );
        });
    ...
    test.describe('API Fallback Mechanisms', () => {
        test('should fallback to direct Apollo.io API when N8N fails', async ({ page }) => {
            ...
            const fallbackIndicator = page.locator(
                '[data-testid="fallback-indicator"], .fallback-notice'
            );
            if (await fallbackIndicator.isVisible()) {
                const indicatorText = await fallbackIndicator.textContent();
                expect(indicatorText?.toLowerCase()).toMatch(
                    /(fallback|direct.*api|alternative.*method)/
                );
            }
        });
    ...
    test.describe('Network and Connectivity Issues', () => {
        test('should handle network disconnection', async ({ page }) => {
            ...
            const offlineIndicator = page.locator(
                '[data-testid="offline-indicator"], .offline-warning'
            );
            await expect(offlineIndicator).toBeVisible({ timeout: 10000 });

            const offlineText = await offlineIndicator.textContent();
            expect(offlineText?.toLowerCase()).toMatch(
                /(offline|connection.*lost|network.*unavailable)/
            );
            ...
            const onlineIndicator = page.locator(
                '[data-testid="online-indicator"], .connection-restored'
            );
        ...
        test('should handle intermittent connectivity', async ({ page }) => {
            ...
            const successMessages = page.locator(
                '[data-testid="chat-message"]:has-text("success")'
            );
    ...
    test.describe('Authentication and Permission Errors', () => {
        test('should handle expired authentication tokens', async ({ page }) => {
            ...
            expect(errorText?.toLowerCase()).toMatch(
                /(session.*expired|please.*login|authentication.*required)/
            );
        });
        ...
        test('should handle API key issues', async ({ page }) => {
            ...
            expect(errorText?.toLowerCase()).toMatch(
                /(api.*key|configuration.*issue|contact.*administrator)/
            );
        });
    });
    ...
    test.describe('Data Validation and Format Errors', () => {
        ...
        test('should handle corrupted response data', async ({ page }) => {
            ...
            expect(responseText?.toLowerCase()).toMatch(
                /(error|issue|unable.*process|please.*try)/
            );
        });
    });
    ...
    test.describe('Recovery and Resilience', () => {
        test('should maintain application state during errors', async ({ page }) => {
            ...
            const successMessage = page.locator(
                '[data-testid="chat-message"]:has-text("Successful query")'
            );
        ...
        test('should provide helpful error recovery suggestions', async ({ page }) => {
            ...
            const recoveryOptions = page.locator(
                '[data-testid="recovery-options"], .error-actions'
            );
            if (await recoveryOptions.isVisible()) {
                const retryButton = recoveryOptions.locator(
                    '[data-testid="retry-button"], button:has-text("retry")'
                );
                const contactButton = recoveryOptions.locator(
                    '[data-testid="contact-support"], button:has-text("support")'
                );

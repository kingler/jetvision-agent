...
test.describe('Conversation Flow Tests', () => {
  ...
  test.describe('System Prompt Activation and Context Switching', () => {
    test('should activate appropriate system prompts based on query intent', async ({ page }) => {
  ...
  test.describe('Follow-up Questions and Context Retention', () => {
    test('should handle follow-up questions maintaining conversation context', async ({ page }) => {
    ...
    test('should maintain conversation history across complex multi-turn dialogues', async ({ page }) => {
    ...
    test('should handle ambiguous follow-ups with context clarification', async ({ page }) => {
      await page.route(WEBHOOK_URL + '**', async (route, request) => {
        ...
        if (payload.message.toLowerCase().includes('check availability')) {
          ...
          response = 'Which specific aircraft would you like to book? Please specify the tail number or hourly rate.';
        } else if (payload.message.toLowerCase().includes('the first one')) {
          response = 'Booking N123GS (the first G650 option) for your Miami to NYC flight tomorrow at 8am';
        }
  ...
  async function simulateConversationFlow(page: Page, flow: any[]) {
    ...
    for (let i = 0; i < flow.length; i++) {
      ...
      if (step.expectedResponse) {
        await expect(page.locator(`text=${step.expectedResponse}`)).toBeVisible({ timeout: 5000 });
      }
  ...
  function detectContext(message: string): string {
    ...
    if (lowerMessage.includes('aircraft') || lowerMessage.includes('flight') || lowerMessage.includes('charter')) {
      return 'avinode';
    }
    if (lowerMessage.includes('contact') || lowerMessage.includes('lead') || lowerMessage.includes('apollo') || lowerMessage.includes('campaign')) {
      return 'apollo';
    }
    if (lowerMessage.includes('travel') || lowerMessage.includes('itinerary') || lowerMessage.includes('planning')) {
      return 'travel';
    }
    if (lowerMessage.includes('analytics') || lowerMessage.includes('metrics') || lowerMessage.includes('conversion') || lowerMessage.includes('roi')) {
      return 'analytics';
    }

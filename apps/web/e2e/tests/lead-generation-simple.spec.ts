...
test.describe('Lead Generation Simple Test', () => {
    ...
    test('should display N8N response for lead generation query in message thread', async ({ page }) => {
        ...
        const submitButton = page.locator('button[type="submit"], [data-testid="send-button"], button:has-text("Send")').first();
        ...
        const chatMessages = page.locator('[data-testid="chat-message"], .message, [class*="message"], [class*="chat"]').all();
    ...
    test('should handle basic chat interface interaction', async ({ page }) => {
        ...
        const clickableElements = await page.locator('button, input, [contenteditable], textarea, a').all();

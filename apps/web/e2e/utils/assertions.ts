...
export class AviationAssertions {
  ...
  async assertValidFlightRoute(routeLocator: Locator): Promise<void> {
    ...
    const distance = routeLocator.locator('[data-testid="route-distance"], [data-testid="flight-time"]');
  ...
  async assertChatStreaming(messageLocator: Locator, timeout: number = 30000): Promise<void> {
    ...
    const streamingIndicator = messageLocator.locator('[data-testid="streaming-indicator"], .streaming');
   ...
   */
  async assertResponseTime(startTime: number, maxTimeMs: number, operationType: string): Promise<void> {
...
export class PerformanceAssertions {
   ...
   */
  static async assertAPIPerformance(page: Page, apiPattern: string, maxResponseTime: number = 5000): Promise<void> {
    ...
    if (responses.length > 0) {
      ...
      console.log(`Average API response time: ${avgResponse.toFixed(2)}ms (max: ${maxResponseTime}ms)`);
    }
  }
}

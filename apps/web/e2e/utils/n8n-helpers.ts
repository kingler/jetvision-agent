...
export class N8NHelper {
  ...
  constructor(private page: Page, config?: Partial<N8NMockConfig>) {
    this.mockConfig = {
  ...
  async setupStreamingMocks(): Promise<void> {
    await this.page.route('**/webhook/jetvision-agent**', async (route) => {
      ...
      if (query.toLowerCase().includes('aircraft') || query.toLowerCase().includes('avinode')) {
        responseType = 'avinode';
      } else if (query.toLowerCase().includes('health') || query.toLowerCase().includes('system')) {
        responseType = 'system';
      }
  ...
  private generateMockResponse(query: string): N8NResponse {
    ...
    if (lowerQuery.includes('aircraft') || lowerQuery.includes('gulfstream') || lowerQuery.includes('avinode')) {
      return getResponseById('avinode-aircraft-success') || this.createGenericResponse('avinode');
    }

...
function generateAlerts(
    ...
    if (conversionRate && conversionRate.value < 0.15) {
        alerts.push(
            `Lead conversion rate is ${(conversionRate.value * 100).toFixed(1)}% (target: >15%)`
        );
    }
    ...
    if (availability && availability.value < 0.999) {
        alerts.push(
            `System availability is ${(availability.value * 100).toFixed(2)}% (target: >99.9%)`
        );
    }
...
export async function POST(request: Request) {
    try {
        ...
        return NextResponse.json({ error: 'Failed to record custom metric' }, { status: 500 });
    }
}

...
export class CompanyRepository {
    ...
    static async getByDomain(domain: string): Promise<Company | null> {
        const result = await db
            .select()
            .from(companies)
            .where(and(eq(companies.domain, domain), isNull(companies.deletedAt)))
            .limit(1);
    ...
    static async getByApolloAccountId(apolloAccountId: string): Promise<Company | null> {
        const result = await db
            .select()
            .from(companies)
            .where(and(eq(companies.apolloAccountId, apolloAccountId), isNull(companies.deletedAt)))
            .limit(1);
    ...
    static async searchByName(searchTerm: string, limit: number = 20): Promise<Company[]> {
        return await db
            .select()
            .from(companies)
            .where(and(ilike(companies.name, `%${searchTerm}%`), isNull(companies.deletedAt)))
            .limit(limit);
    }
    ...
    static async getByIndustry(industry: string): Promise<Company[]> {
        return await db
            .select()
            .from(companies)
            .where(and(eq(companies.industry, industry), isNull(companies.deletedAt)));
    }
    ...
    static async getBySize(
        ...
        return await db
            .select()
            .from(companies)
            .where(and(eq(companies.size, size), isNull(companies.deletedAt)));
    }

    /**
     * Get company with users and leads count
     */
    static async getWithStats(id: string): Promise<
        | (Company & {
              userCount: number;
              leadCount: number;
          })
    ...
    > {
        ...
        const leadCountResult = await db
            .select()
            .from(leads)
            .where(and(eq(leads.companyId, id), isNull(leads.deletedAt)));
    ...
    static async getAll(
        ...
        const totalResult = await db.select().from(companies).where(isNull(companies.deletedAt));

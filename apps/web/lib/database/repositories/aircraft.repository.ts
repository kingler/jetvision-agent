...
export class AircraftRepository {
    ...
    static async getByAvinodeId(avinodeId: string): Promise<Aircraft | null> {
        const result = await db
            .select()
            .from(aircraft)
            .where(and(eq(aircraft.avinodeId, avinodeId), isNull(aircraft.deletedAt)))
            .limit(1);
    ...
    static async searchWithFilters(filters: {
        ...
        const conditions = [eq(aircraft.availability, availability), isNull(aircraft.deletedAt)];
        ...
        const totalResult = await db.select().from(aircraft).where(whereCondition);
    ...
    static async getByOperator(operatorName: string): Promise<Aircraft[]> {
        return await db
            .select()
            .from(aircraft)
            .where(and(eq(aircraft.operatorName, operatorName), isNull(aircraft.deletedAt)))
    ...
    static async bulkCreate(aircraftData: NewAircraft[]): Promise<Aircraft[]> {
        ...
        return await db.insert(aircraft).values(aircraftWithTimestamps).returning();
    }

    /**
     * Get aircraft with booking statistics
     */
    static async getWithBookingStats(id: string): Promise<
        | (Aircraft & {
    ...
    > {
        ...
        const allBookings = await db
            .select()
            .from(bookings)
            .where(and(eq(bookings.aircraftId, id), isNull(bookings.deletedAt)));
    ...
    > {
        ...
        for (const aircraftRecord of allAircraft) {
            ...
            const utilizationHours = aircraftBookings.reduce(
                (sum, booking) => sum + (booking.flightHours || 0),
                0
            );

            const revenue = aircraftBookings.reduce(
                (sum, booking) => sum + (booking.totalCost || 0),
                0
            );
    ...
    static async getNearbyAircraft(
        ...
        const conditions = [eq(aircraft.availability, 'AVAILABLE'), isNull(aircraft.deletedAt)];

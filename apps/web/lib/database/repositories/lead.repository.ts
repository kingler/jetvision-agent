...
export class LeadRepository {
  ...
  static async getByApolloContactId(apolloContactId: string): Promise<Lead | null> {
    const result = await db
      ...
      .from(leads)
      .where(and(
        eq(leads.apolloContactId, apolloContactId),
        isNull(leads.deletedAt)
      ))
      .limit(1);
  ...
  static async getByStatus(
    status: 'NEW' | 'CONTACTED' | 'QUALIFIED' | 'PROPOSAL_SENT' | 'NEGOTIATING' | 'CLOSED_WON' | 'CLOSED_LOST' | 'UNRESPONSIVE',
    limit: number = 100
  ): Promise<Lead[]> {
    return await db
      .select()

...
const INTENT_PATTERNS = {
  'apollo_search': {
    keywords: ['apollo', 'contact', 'executive', 'lead', 'prospect', 'search', 'find people', 'email', 'outreach'],
    weight: 0.8,
    businessContext: 'apollo' as const
  },
  'aircraft_availability': {
    keywords: ['aircraft', 'jet', 'availability', 'charter', 'flight', 'avinode', 'plane', 'booking'],
    weight: 0.8,
    businessContext: 'avinode' as const
  },
  'fleet_metrics': {
    keywords: ['utilization', 'metrics', 'performance', 'analytics', 'dashboard', 'kpi', 'fleet'],
    weight: 0.7,
    businessContext: 'avinode' as const
  },
  'campaign_management': {
...
const VISUALIZATION_KEYWORDS = ['chart', 'graph', 'dashboard', 'metrics', 'analytics', 'visual', 'plot', 'report'];

// Recommendation triggers
const RECOMMENDATION_KEYWORDS = ['recommend', 'suggest', 'advice', 'best', 'optimize', 'improve', 'strategy'];
...
export function detectIntent(message: string): DetectedIntent {
  ...
  let detectedBusinessContexts: Set<string> = new Set();
  ...
  for (const [intentName, pattern] of Object.entries(INTENT_PATTERNS)) {
    ...
    const normalizedScore = matchedKeywords.length > 0 ? 
      (score / pattern.keywords.length) * (matchedKeywords.length / pattern.keywords.length) : 0;
...
function extractEntities(message: string): string[] {
  ...
  // Aircraft types
  const aircraftTypes = ['Citation', 'Gulfstream', 'Challenger', 'Falcon', 'Hawker', 'King Air', 'Phenom'];
  ...
  const companyMatch = message.match(/\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s+(?:Inc|Corp|LLC|Ltd|Company)/gi);
  if (companyMatch) {

 ...
 */
export function startEditingPrompt(prompt: EditablePrompt, newText?: string): EditablePrompt {
    return {
        ...prompt,
...
export function getCurrentPromptText(prompt: EditablePrompt): string {
    return prompt.isEditing ? prompt.editedText || prompt.originalText : prompt.originalText;
}
...
export function isPromptModified(prompt: EditablePrompt): boolean {
    return prompt.editedText !== undefined && prompt.editedText !== prompt.originalText;
}
...
export function generateEditSuggestions(originalText: string): string[] {
    ...
    if (
        originalText.toLowerCase().includes('plane') &&
        !originalText.toLowerCase().includes('aircraft')
    ) {
        suggestions.push('Consider using "aircraft" for more professional aviation terminology');
    }

    if (
        originalText.toLowerCase().includes('cheap') ||
        originalText.toLowerCase().includes('discount')
    ) {
        suggestions.push(
            'For executive aviation, consider "cost-effective" or "competitive pricing"'
        );
    }
...
export function enhancePromptForAviation(text: string): {
    ...
    if (hasAviationKeywords && enhancedText.length < 50) {
        const contextAddition =
            ' Please provide information specific to business aviation and executive travel services.';
        if (
            !enhancedText.toLowerCase().includes('business') &&

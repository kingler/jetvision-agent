...
const ALL_AVIATION_KEYWORDS = Object.values(AVIATION_KEYWORDS)
    .flat()
...
export function getAviationContext(message: string): {
    ...
    if (classification.isAviation) {
        ...
        if (
            classification.categories.includes('aircraft') ||
            classification.categories.includes('services')
        ) {
            suggestedActions.push('Search aviation databases');
        }
        if (
            classification.categories.includes('airports') ||
            classification.categories.includes('travel')
        ) {

...
export class CircuitBreakerError extends Error {
  constructor(message: string, public readonly circuitOpen: boolean = false) {
    super(message);
...
export class CircuitBreaker<T = any> {
  ...
  public getHealthMetrics() {
    ...
    return {
      ...
      timeSinceLastFailure: this.state.lastFailureTime ? now - this.state.lastFailureTime : null,
      timeToNextAttempt: this.state.nextAttemptTime > now ? this.state.nextAttemptTime - now : 0,
    };
  }

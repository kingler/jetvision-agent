...
export class N8NClient {
     ...
     */
    private async handleLeadProcessingFallback(
        payload: N8NWebhookPayload
    ): Promise<N8NFallbackResponse> {
     ...
     */
    private async handleEmailResponseFallback(
        payload: N8NWebhookPayload
    ): Promise<N8NFallbackResponse> {
     ...
     */
    private async handleBookingRequestFallback(
        payload: N8NWebhookPayload
    ): Promise<N8NFallbackResponse> {
        // Booking requests are critical - might need immediate attention

        Sentry.captureMessage(
            'Booking request fallback activated - manual intervention may be required',
            {
                level: 'warning',
                tags: {

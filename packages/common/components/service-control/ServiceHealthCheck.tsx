...
export const ServiceHealthCheck = ({
    ...
    const { services, performHealthCheck, updateHealthCheckResult, isServiceAvailable } =
    ...
    const handleHealthCheck = async () => {
        ...
        try {
            ...
            updateHealthCheckResult(serviceId, result.healthy, result.error);
        } catch (error) {
        ...
        <div className={`bg-background rounded-lg border p-4 ${className}`}>
            <div className="mb-4 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <ServiceStatusIndicator status={service?.status || 'disabled'} size="sm" />
                </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
                                ...
                                <span
                                    className={`font-medium ${
                                        lastResult.status === 'healthy'
                                            ? 'text-green-600'
                                            : lastResult.status === 'degraded'
                                              ? 'text-yellow-600'
                                              : 'text-red-600'
                                    }`}
                                >
                                    {lastResult.status.charAt(0).toUpperCase() +
                                        lastResult.status.slice(1)}
                                </span>
                            </div>
                            ...
                            <div>
                                ...
                                <span className="font-medium">
                                    {formatLatency(lastResult.latency)}
                                </span>
                            </div>

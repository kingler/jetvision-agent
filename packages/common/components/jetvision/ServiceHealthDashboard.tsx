...
import { Card, CardContent, CardHeader, CardTitle, Badge, Button, Progress } from '@repo/ui';
import {
  IconActivity,
  IconAlert,
  IconAlertTriangle,
...
export const ServiceHealthDashboard: React.FC<ServiceHealthDashboardProps> = ({
  ...
  const getStatusColor = (state: string): string => {
    switch (state) {
      ...
      case 'HALF_OPEN': return 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300';
  ...
  const getStatusIcon = (state: string) => {
    switch (state) {
      case 'CLOSED': return <IconCheck className="h-4 w-4" />;
      case 'HALF_OPEN': return <IconAlertTriangle className="h-4 w-4" />;
      case 'OPEN': return <IconX className="h-4 w-4" />;
      default: return <IconAlert className="h-4 w-4" />;
    }
  };
  ...
  const handleServiceAction = (serviceName: string, action: 'reset' | 'force-open' | 'force-close') => {
    if (onServiceAction) {
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            ...
            Uptime: {formatUptime((Date.now() - systemHealth.uptime) / 1000)}
          </p>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-blue-600">{systemHealth.cacheStats.hitRate.toFixed(1)}%</div>
              <div className="text-sm text-muted-foreground">Cache Hit Rate</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600">{systemHealth.cacheStats.totalEntries.toLocaleString()}</div>
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-lg flex items-center gap-2">
                  ...
                  {service.serviceName.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                </CardTitle>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <div className="font-semibold">
                    {((service.successCount / service.requestCount) * 100).toFixed(1)}%
                  </div>
                </div>
                <div>
                  <div className="flex items-center gap-1 text-muted-foreground">
                  ...
                  <div className="font-semibold">{service.averageResponseTime}ms</div>
                </div>
                <div>
                  <div className="flex items-center gap-1 text-muted-foreground">
              ...
              <div className="text-xs text-muted-foreground">
                {service.lastSuccessTime && (
                  <div>Last success: {new Date(service.lastSuccessTime).toLocaleTimeString()}</div>
                )}
                {service.lastFailureTime && (
                  <div>Last failure: {new Date(service.lastFailureTime).toLocaleTimeString()}</div>
                )}
              </div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                {onServiceAction && (
                  <Button 
                    variant="ghost" 
                    size="sm" 
                    onClick={() => handleServiceAction(service.serviceName, service.state === 'OPEN' ? 'force-close' : 'force-open')}
                  >
                        ...
                        <div className="flex items-center gap-2">
                          <Badge 
                            variant={alert.level === 'critical' ? 'destructive' : 
                                   alert.level === 'warning' ? 'warning' : 'secondary'}
                            className="text-xs"
                          >
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="text-center">
              ...
              <Progress value={systemHealth.cacheStats.hitRate} className="mt-2 h-2" />
            </div>
            <div className="text-center">

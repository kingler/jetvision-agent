...
export const PromptCardsModal: React.FC<PromptCardsModalProps> = ({
    ...
    const { prompts: allPromptsWithCustom, createPrompt, updatePrompt } = usePromptManager();

    // Use combined prompts from usePromptManager (includes custom prompts)
    const allPrompts = allPromptsWithCustom.map(prompt => {
        // Find category for static prompts to get icon and color
        const category = STATIC_PROMPTS_DATA.find(cat => cat.prompts.some(p => p.id === prompt.id));
        return {
            ...prompt,
            categoryIcon: category?.icon || IconEdit, // Default icon for custom
    ...
    // Handler for creating new prompt
    ...
    // Handler for editing existing prompt
    const handleEditPrompt = useCallback((prompt: PromptCard) => {
        setEditorMode('edit');
        setEditingPrompt(prompt);
        setIsEditorOpen(true);
    }, []);

    // Handler for saving prompt (create or update)
    const handleSavePrompt = useCallback(
        async (title: string, prompt: string, fullPrompt: string, description: string) => {
            try {
                if (editorMode === 'create') {
                    ...
                    await updatePrompt(editingPrompt.id, title, prompt, fullPrompt, description);
                }
        ...
        [editorMode, editingPrompt, createPrompt, updatePrompt]
    );
                    ...
                    >
                        ...
                        <div className="pt-mobile-safe-top flex items-center justify-between border-b border-gray-200 p-4 sm:p-6 dark:border-gray-800">
                            <div className="flex items-center gap-3">
                                ...
                                {/* Create New Button - Following Monochromatic Style */}
                                <Button
                                    onClick={handleCreateNew}
                                    variant="brand" // Uses --brand (black/dark gray)
                                    size="sm"
                                    prefixIcon={IconPlus}
                                    className="ml-4"
                                >
                                                        ...
                                                        <div className="flex flex-1 items-start gap-3">
                                                            ...
                                                            <div className="flex-1">
                                                                ...
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    {/* Custom prompt indicator - Monochromatic style */}
                                                                    {(prompt as any).isCustom && (
                                                                        <span className="text-mobile-xs bg-muted text-muted-foreground inline-flex items-center rounded-md px-2 py-1 font-medium sm:text-xs">
                                                                            Custom
                                                                        </span>
                                                                    )}
                                                                </div>

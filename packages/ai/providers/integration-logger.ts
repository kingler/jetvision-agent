...
export class IntegrationLogger {
   ...
   * Log debug message
   */
  debug(category: string, message: string, metadata?: Record<string, any>, requestId?: string): string {
   ...
   * Log info message
   */
  info(category: string, message: string, metadata?: Record<string, any>, requestId?: string): string {
   ...
   * Log warning message
   */
  warn(category: string, message: string, metadata?: Record<string, any>, requestId?: string): string {
   ...
   * Log error message
   */
  error(category: string, message: string, error?: Error, metadata?: Record<string, any>, requestId?: string): string {
    const logMetadata = {
      ...metadata,
      ...(error && {
   ...
   */
  critical(category: string, message: string, error?: Error, metadata?: Record<string, any>, requestId?: string): string {
    const logMetadata = {
      ...metadata,
      ...(error && {
        errorName: error.name,
   ...
   * Log request start
   */
  startRequest(category: string, operation: string, metadata?: Record<string, any>): {
    requestId: string;
    startTime: number;
    ...
    return {
      ...
      complete: (success: boolean, result?: any, error?: Error) => {
        ...
        this.log(level, category, `${status} Completed ${operation}`, {
          ...metadata,
          operation,
          success,
        ...
        }, requestId, error?.stack, duration);
      }
    };
  }
  ...
  logApiCall(
    ...
    return this.log(level, 'api-call', `${status} ${method} ${url}`, {
      service,
      method,
      url,
  ...
  logCircuitBreakerEvent(
    ...
    const level = newState === 'OPEN' ? LogLevel.WARN : 
                 newState === 'CLOSED' ? LogLevel.INFO : LogLevel.DEBUG;

    return this.log(level, 'circuit-breaker', `Circuit breaker ${service}: ${oldState} → ${newState}`, {
      service,
      oldState,
      newState,
      reason,
  ...
  logCacheOperation(
    ...
    return this.debug('cache', `${emoji[operation]} Cache ${operation} - ${cacheType}`, {
      operation,
      cacheType,
  ...
  logFallbackActivation(
    ...
    return this.warn('fallback', `🔄 Fallback activated: ${primaryService} → ${fallbackService}`, {
      primaryService,
      fallbackService,
      reason,
      ...metadata
  ...
  logServiceRecovery(
    ...
    return this.info('recovery', `🟢 Service recovered: ${service}`, {
      service,
    ...
    if (filter) {
      filteredLogs = this.logs.filter(log => {
        ...
        if (filter.searchText && 
            ...
            !JSON.stringify(log.metadata || {}).toLowerCase().includes(filter.searchText.toLowerCase())
        ) return false;
   ...
   */
  getPerformanceMetrics(service?: string, hours = 1): {
    ...
    const avgResponseTime = relevantLogs.reduce((sum, log) => sum + (log.duration || 0), 0) / totalRequests;
...
export const DEFAULT_LOGGER_CONFIG: LoggerConfig = {
  ...
  maxLogEntries: 10000,
  categories: [
    'system', 'api-call', 'circuit-breaker', 'cache', 'fallback', 
    'recovery', 'n8n', 'apollo', 'avinode', 'llm'
  ],

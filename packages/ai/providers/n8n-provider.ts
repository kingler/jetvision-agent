...
class N8nAgentModelImpl implements N8nAgentModel {
    ...
    async doGenerate(
        ...
        try {
            ...
            throw new Error(
                `n8n webhook error: ${error instanceof Error ? error.message : 'Unknown error'}`
            );
        }
    }
    ...
    private formatResponse(response: any): string {
        ...
        if (response?.result) {
            return typeof response.result === 'string'
                ? response.result
                : this.formatStructuredData(response.result);
        }
        ...
        if (Array.isArray(response) && response.length > 0) {
            ...
            if (firstItem?.result)
                return typeof firstItem.result === 'string'
                    ? firstItem.result
                    : this.formatStructuredData(firstItem.result);

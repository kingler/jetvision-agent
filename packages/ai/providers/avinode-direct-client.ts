...
export class AvinodeDirectClient {
    ...
    constructor(
        ...
        this.circuitBreaker =
            circuitBreaker ||
            new CircuitBreaker('avinode-direct', {
                failureThreshold: 3,
    ...
    async searchAircraft(params: AircraftSearchParams): Promise<AircraftSearchResult> {
        const cacheKey = this.cache ? CacheManager.generateKey('avinode-search', params) : null;
     ...
     */
    async getFlightQuote(
        aircraftId: string,
        searchParams: AircraftSearchParams
    ): Promise<FlightQuote> {
        const cacheKey = this.cache
            ? CacheManager.generateKey('avinode-quote', { aircraftId, ...searchParams })
            : null;
    ...
    async getFleetStatus(operatorId?: string): Promise<FleetStatus> {
        const cacheKey = this.cache
            ? CacheManager.generateKey('avinode-fleet', { operatorId })
            : null;
    ...
    async searchEmptyLegs(params: {
        ...
        const cacheKey = this.cache ? CacheManager.generateKey('avinode-empty-legs', params) : null;
    ...
    private parseQuoteResponse(response: any): FlightQuote {
        return {
            ...
            pricing: {
                ...
                positioning: response.pricing.positioning
                    ? this.formatCurrency(response.pricing.positioning)
                    : undefined,
                overnight: response.pricing.overnight
                    ? this.formatCurrency(response.pricing.overnight)
                    : undefined,
    ...
    private generateRecommendations(aircraft: Aircraft[], params: AircraftSearchParams): string[] {
        ...
        if (aircraft.length === 0) {
            recommendations.push(
                'No aircraft found for your criteria. Try expanding your search dates or destinations.'
            );
        ...
        const prices = aircraft
            .map(ac => parseFloat(ac.hourlyRate.replace(/[$,]/g, '')))
            .sort((a, b) => a - b);
        const medianPrice = prices[Math.floor(prices.length / 2)];
        const cheapest = aircraft.find(
            ac => parseFloat(ac.hourlyRate.replace(/[$,]/g, '')) === prices[0]
        );

        if (cheapest) {
            recommendations.push(
                `Most economical option: ${cheapest.fullName} at ${cheapest.hourlyRate}/hour`
            );
        }
        ...
        if (distance > 3000) {
            const heavyJets = aircraft.filter(
                ac => ac.category === 'Heavy Jet' || ac.category === 'Ultra Long Range'
            );
            if (heavyJets.length > 0) {
                recommendations.push(
                    `For this long-range flight, consider heavy jets for better comfort and fuel efficiency`
                );
            }
            ...
            if (lightJets.length > 0) {
                recommendations.push(
                    `For this short flight, light jets offer excellent value and convenience`
                );
            }
        }

        // Empty legs recommendations
        const emptyLegs = aircraft.filter(
            ac => ac.totalCost && parseFloat(ac.totalCost.replace(/[$,]/g, '')) < medianPrice * 0.7
        );
        if (emptyLegs.length > 0) {
            recommendations.push(
                `${emptyLegs.length} empty leg opportunities available with significant savings`
            );
        }

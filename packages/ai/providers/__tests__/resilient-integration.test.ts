...
import { JetVisionResilientProvider, createJetVisionResilient } from '../jetvision-resilient-provider';
...
describe('JetVision Resilient Provider Integration Tests', () => {
    ...
    mockOptions = {
      prompt: {
        messages: [
          {
            role: 'user',
            content: [{ type: 'text', text: 'Find executive assistants in tech companies' }]
          }
        ]
      },
      mode: { type: 'regular' }
          ...
          people: [
            {
              ...
              organization: { name: 'Big Tech Inc', estimated_num_employees: 1000 },
      ...
      const avinodeOptions = {
        ...
        prompt: {
          messages: [
            {
              role: 'user',
              content: [{ type: 'text', text: 'Show me available aircraft from JFK to LAX' }]
            }
          ]
        }
      };
  ...
  describe('Cache Fallback', () => {
    ...
    it('should serve cached responses when all APIs are down', async () => {
      ...
      const cacheKey = 'fallback:' + Buffer.from('Find executive assistants in tech companies').toString('base64url');
  ...
  describe('Circuit Breaker Integration', () => {
    it('should track failures and open circuit breaker', async () => {
      ...
      const n8nCircuitBreaker = healthReport.circuitBreakerStatus.find(cb => cb.serviceName.includes('n8n'));
  ...
  describe('Streaming Support', () => {
    it('should stream N8N responses successfully', async () => {
      mockFetch.mockResolvedValueOnce(
        new Response(JSON.stringify({
          message: 'Streaming response from N8N workflow with detailed lead information'
        }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' }
        })
      ...
      const apolloQuery = {
        ...
        prompt: {
          messages: [
            {
              role: 'user',
              content: [{ type: 'text', text: 'Find executive assistants using Apollo campaign data' }]
            }
          ]
        }
      };
      ...
      const avinodeQuery = {
        ...
        prompt: {
          messages: [
            {
              role: 'user',
              content: [{ type: 'text', text: 'Show fleet status and aircraft availability' }]
            }
          ]
        }
      };
  ...
  describe('Error Handling and Recovery', () => {
    ...
    it('should maintain service metrics during failures', async () => {
      ...
      expect(updatedMetrics.totalRequests).toBe(initialMetrics.totalRequests + 1);
      expect(updatedMetrics.fallbackActivations).toBeGreaterThanOrEqual(initialMetrics.fallbackActivations);
    });

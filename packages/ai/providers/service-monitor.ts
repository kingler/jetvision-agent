...
export class ServiceMonitor {
  ...
  private updateServiceMetrics(service: ServiceHealth, timestamp: number): void {
    ...
    const metrics: ServiceMetrics = {
      ...
      successRate: service.requestCount > 0 ? 
        ((service.requestCount - service.failureCount) / service.requestCount) * 100 : 100,
      lastError: existingMetrics?.lastError
    };

    // Update last error if there are new failures
    if (service.lastFailureTime && 
        (!existingMetrics?.lastError || service.lastFailureTime > existingMetrics.lastError.timestamp)) {
      metrics.lastError = {
  ...
  private checkServiceThresholds(service: ServiceHealth): void {
    ...
    if (metrics.successRate < (100 - this.config.alertThresholds.errorRate)) {
      ...
      if (!existingAlert) {
        this.addAlert(
          ...
          { errorRate: 100 - metrics.successRate, threshold: this.config.alertThresholds.errorRate }
        );
      }
    }
    ...
    if (service.state === 'OPEN') {
      ...
      const downAlerts = this.alerts.filter(a => 
        a.serviceName === service.serviceName && 
        a.message.includes('service down') && 
        !a.resolved
      );
  ...
  private attemptAutoRecovery(): void {
    ...
    const failedServices = circuitBreakerManager.getAllServiceHealth()

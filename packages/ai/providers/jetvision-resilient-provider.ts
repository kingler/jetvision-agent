...
export class JetVisionResilientProvider implements LanguageModelV1 {
  ...
  constructor(modelId: string, private config: Required<ResilientProviderConfig>) {
  ...
  private async executeWithFallback(
    ...
    isStreaming: boolean
  ): Promise<{ text: string; usage: any; finishReason: LanguageModelV1FinishReason }> {
    ...
    try {
      ...
      console.log('🟡 N8N primary failed, trying direct APIs...', error.message);
    ...
    try {
      ...
      console.log('🟡 Direct APIs failed, checking cache...', error.message);
    }
    ...
    try {
      ...
      console.log('🟡 Cache fallback failed', error.message);
    }
    ...
    return this.n8nCircuitBreaker.execute(async () => {
      const response = await fetch(this.config.n8nWebhookUrl, {
        ...
        headers: {
          ...
          ...(this.config.n8nApiKey && { 'Authorization': `Bearer ${this.config.n8nApiKey}` })
        },
        body: JSON.stringify({
          prompt: query,
          context: {
  ...
  private async tryDirectAPIs(query: string, source: string, options: any): Promise<string> {
    ...
    if (source === 'apollo' && this.apolloClient) {
      ...
      if (this.apolloClient && (lowerQuery.includes('lead') || lowerQuery.includes('campaign'))) {
        return this.handleApolloQuery(query, lowerQuery);
      } else if (this.avinodeClient && (lowerQuery.includes('aircraft') || lowerQuery.includes('flight'))) {
        return this.handleAvinodeQuery(query, lowerQuery);
      } else {
  ...
  private async handleApolloQuery(query: string, lowerQuery: string): Promise<string> {
    ...
    try {
      ...
      throw new Error(`Apollo.io direct API failed: ${error.message}`);
    }
  }
  ...
  private async handleAvinodeQuery(query: string, lowerQuery: string): Promise<string> {
    ...
    try {
      ...
      throw new Error(`Avinode direct API failed: ${error.message}`);
    }
  }
  ...
  private async tryCacheFallback(query: string, source: string): Promise<string | null> {
    const cacheType = source === 'apollo' ? 'apollo-leads' : 
                     source === 'avinode' ? 'avinode-aircraft' : 
                     'llm-responses';
...
*Last health check: ${new Date(this.serviceStatus.lastHealthCheck).toLocaleTimeString()}*`;
   ...
   */
  private async streamResponse(text: string, controller: ReadableStreamDefaultController<LanguageModelV1StreamPart>): Promise<void> {
  ...
  private updateMetrics(responseTime: number, success: boolean): void {
    ...
    if (success) {
      this.metrics.successRate = ((currentSuccessRate * (totalRequests - 1)) + 100) / totalRequests;
    } else {
      this.metrics.successRate = ((currentSuccessRate * (totalRequests - 1)) + 0) / totalRequests;
    }
    
    // Update average response time (simple moving average of last 100 requests)
    this.metrics.averageResponseTime = ((this.metrics.averageResponseTime * Math.min(totalRequests - 1, 99)) + responseTime) / 
  ...
  private async performHealthCheck(): Promise<void> {
    ...
    results.forEach((result, index) => {
      const serviceName = healthStatuses[index] as keyof ServiceStatus;
      this.serviceStatus[serviceName] = result.status === 'fulfilled' && result.value ? 'healthy' : 'down';
    });

    // Update overall health
    const healthyServices = Object.entries(this.serviceStatus)
      .filter(([key, value]) => key !== 'overallHealth' && key !== 'lastHealthCheck' && value === 'healthy')
      .length;
  ...
  private async checkN8nHealth(): Promise<boolean> {
    try {
      const response = await fetch(this.config.n8nWebhookUrl.replace('/webhook/', '/health'), {
        method: 'GET',

                ...
                try {
                    ...
                    if (self.config.preferN8n) {
                        try {
                            ...
                            console.log(
                                '⚠️ n8n failed, falling back to direct LLM for streaming:',
                                error
                            );
                        ...
                        try {
                            ...
                            console.log(
                                '⚠️ Direct LLM failed, falling back to n8n for streaming:',
                                error
                            );
                            text = await self.tryN8nFirst(query, abortSignal);
                        }
    ...
    private async tryN8nFirst(query: string, abortSignal?: AbortSignal): Promise<string> {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    `n8n webhook failed (${response.status}): ${errorText || 'Unknown error'}`
                );
            }
...
export function createJetVisionHybrid(config: JetVisionHybridConfig = {}) {
    return {
        jetvisionAgent: (modelId: string = 'jetvision-hybrid-v1') =>
            new JetVisionHybridModelImpl(modelId, config),
    };
}

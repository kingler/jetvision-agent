...
export class CacheManager {
  ...
  constructor(private config: CacheConfig) {
    this.cache = new LRUCache({ maxSize: config.maxSize });
    this.metrics = {
      hits: 0,
      misses: 0,
      entries: 0,
   ...
   */
  set<T>(key: string, data: T, source: string, options?: {
    ttl?: number;
    version?: string;
    metadata?: Record<string, any>;
  }): void {
...
export class MultiTierCacheManager {
  ...
  constructor() {
    ...
    this.caches.set('apollo-leads', new CacheManager({
      maxSize: 1000,
    ...
    this.caches.set('apollo-campaigns', new CacheManager({
      maxSize: 500,
    ...
    this.caches.set('avinode-aircraft', new CacheManager({
      maxSize: 2000,
    ...
    this.caches.set('avinode-pricing', new CacheManager({
      maxSize: 1000,
    ...
    this.caches.set('llm-responses', new CacheManager({
      maxSize: 200,
    ...
    this.caches.set('service-health', new CacheManager({
      maxSize: 100,

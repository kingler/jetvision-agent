...
export class ApolloDirectClient {
  ...
  constructor(
    ...
    this.circuitBreaker = circuitBreaker || new CircuitBreaker('apollo-direct', {
      failureThreshold: 3,
  ...
  async searchExecutiveAssistants(params: {
    ...
    const searchParams: ApolloSearchParams = {
      ...
      locations: params.locations || ['New York', 'San Francisco', 'Los Angeles', 'Miami', 'London'],
  ...
  async findFundingCompanies(params: {
    ...
    const searchParams: ApolloSearchParams = {
      personTitles: [
        'CEO', 'COO', 'CFO', 'President', 'Vice President',
        'Managing Director', 'Partner', 'Founder'
      ],
      industryKeywords: params.industryKeywords || [
        'Technology', 'Software', 'Fintech', 'Healthcare',
  ...
  async searchPeople(params: ApolloSearchParams): Promise<ApolloSearchResult> {
    const cacheKey = this.cache ? 
      CacheManager.generateKey('apollo-search', params) : null;
  ...
  async getCampaignMetrics(campaignId: string): Promise<ApolloCampaignMetrics> {
    const cacheKey = this.cache ?
      CacheManager.generateKey('apollo-campaign', { campaignId }) : null;
   ...
   * Enrich contact data
   */
  async enrichContact(identifier: { email?: string; linkedin?: string; domain?: string }): Promise<ApolloEnrichmentData> {
    const cacheKey = this.cache ?
      CacheManager.generateKey('apollo-enrich', identifier) : null;
  ...
  private parseEnrichmentResponse(response: any): ApolloEnrichmentData {
    ...
    return {
      ...
      fundingRounds: person.organization?.funding_events?.map((event: any) => ({
  ...
  private calculateLeadScore(person: any): number {
    ...
    const relevantIndustries = ['technology', 'finance', 'real estate', 'entertainment', 'private equity'];

...
export const defaultProviderConfig: ProviderConfig = {
  ...
  n8n: {
    webhookUrl: process.env.NEXT_PUBLIC_N8N_WEBHOOK_URL || 'http://localhost:5678/webhook/jetvision-agent',
...
export function shouldUseN8nForQuery(query: string): boolean {
  ...
  const n8nKeywords = [
    'apollo', 'campaign', 'lead', 'executive', 'assistant',
    'avinode', 'aircraft', 'fleet', 'availability', 'booking',
    'workflow', 'automation', 'integration', 'sync',
    'dashboard', 'report', 'analytics', 'metrics'
  ];
  
  // Queries better suited for direct LLM
  const llmKeywords = [
    'explain', 'how', 'what', 'why', 'help', 'guide',
    'tutorial', 'example', 'best practice', 'advice',
    'compare', 'difference', 'pros and cons'
  ];
  
  const n8nScore = n8nKeywords.reduce((score, keyword) => 
    lowerQuery.includes(keyword) ? score + 1 : score, 0);

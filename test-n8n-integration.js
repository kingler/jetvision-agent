...
class N8NTester {
    ...
    async testSessionManagement() {
        ...
        for (const test of sessionTests) {
            try {
                ...
                const responseText = await response.text();
                console.log(
                    `✅ Session test passed - Status: ${response.status}, Content: ${responseText.length} chars`
                );
            } catch (error) {
    ...
    async testFrontendHealthCheck() {
        ...
        try {
            ...
            if (healthData.webhook?.circuitBreaker) {
                console.log(
                    `🔌 Circuit Breaker State: ${healthData.webhook.circuitBreaker.isOpen ? 'OPEN (failing)' : 'CLOSED (healthy)'}`
                );
                console.log(
                    `⚡ Failures: ${healthData.webhook.circuitBreaker.failures}/${healthData.webhook.circuitBreaker.threshold}`
                );
            }
        } catch (error) {
    ...
    printSummary() {
        ...
        this.testResults.forEach(result => {
            ...
            console.log(
                `${status} ${result.promptId}: ${time} - ${result.query.substring(0, 50)}...`
            );
        ...
        console.log(
            '3. 🔧 Consider checking N8N workflow configuration for proper response formatting'
        );
